...
exports.SaleService = {
     ...
     */
    calculatePaymentStatus(total, totalPaid) {
        if (totalPaid <= 0)
     ...
     */
    async createSale(dto, actorUserId, branchId) {
        ...
        for (const it of dto.items) {
            if (!it.productId)
                throw { status: 400, message: "Cada item debe tener productId" };
            const quantity = Number(it.quantity);
            if (!Number.isInteger(quantity) || quantity <= 0)
                throw { status: 400, message: "quantity inválida para un item" };
            ...
            const serialNumbers = Array.isArray(it.serialNumbers)
                ? it.serialNumbers.map((serial) => String(serial).trim()).filter(Boolean)
                : [];
        ...
        if (!paymentsDto || paymentsDto.length === 0)
            throw { status: 400, message: "Debe incluir al menos un payment" };
        ...
        const totalPaid = paymentsDto.reduce((s, p) => s + Number(p.amount), 0);
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            if (typeof dto.client === "number" || (typeof dto.client === "string" && /^\d+$/.test(dto.client))) {
                ...
                if (!exists)
                    throw { status: 400, message: `Cliente ${clientId} no existe` };
            }
                ...
                if (dto.client.id_cliente || dto.client.id) {
                    ...
                    const normalizedPhone = (0, phone_1.normalizePhoneNumber)(dto.client.phone ?? dto.client.telefono ?? "");
                    const countryCode = (0, phone_1.normalizeCountryCode)(dto.client.countryCode ?? "591");
            ...
            for (const it of itemsData) {
                ...
                if (availableSerials.length !== it.serialNumbers.length) {
                    const found = new Set(availableSerials.map((item) => item.serialNumber));
                    const missing = it.serialNumbers.filter((serial) => !found.has(serial));
                    throw {
                        status: 400,
                ...
                if (product.priceCurrency !== 'BOB') {
                    if (originalPrice && originalPrice > 0 && originalCurrency !== 'BOB') {
                        ...
                        try {
                            conversionRate = await exchangeRate_service_1.ExchangeRateService.getRate(product.priceCurrency, 'BOB');
                        }
                        catch (error) {
            ...
            if (totalPaid > calculatedTotal) {
                ...
                const cashPaymentIndex = adjustedPayments.findIndex(async (p) => {
                    const method = await paymentMethod_repository_1.PaymentMethodRepository.findById(p.paymentMethodId);
            ...
            for (const p of adjustedPayments) {
                ...
                if (!pm)
                    throw { status: 404, message: `Método de pago ${p.paymentMethodId} no encontrado` };
            ...
            for (const item of itemsToCreate) {
                ...
                await tx.productSerial.updateMany({
                    ...
                    data: {
                        status: client_1.ProductSerialStatus.SOLD,
                        saleId: created.id,
     ...
     */
    async addPayment(addPaymentDto, actorUserId) {
        return await prisma.$transaction(async (tx) => {
            ...
            if (paymentMethod.isCash) {
                if (addPaymentDto.cashBoxId) {
                    ...
                    const openBox = await cashBox_repository_1.CashBoxRepository.findOpen();
                    if (!openBox) {
