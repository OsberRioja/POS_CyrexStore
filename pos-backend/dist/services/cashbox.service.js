...
function calculateProfitData(box) {
    let totalSales = 0;
    let totalCost = 0;
...
exports.CashBoxService = {
    async open(openDto, actorUserId, branchId) {
        ...
        const existing = await cashBox_repository_1.CashBoxRepository.findOpenByBranch(branchId);
        if (existing)
            throw { status: 400, message: "Ya existe una caja abierta en esta sucursal" };
        const created = await cashBox_repository_1.CashBoxRepository.create({
    ...
    async getOpen(branchId) {
        const open = await cashBox_repository_1.CashBoxRepository.findOpenByBranch(branchId);
        if (!open)
            return null;
    ...
    async close(boxId, actorUserId, closeDto) {
        ...
        // Validar existencia
        const box = await cashBox_repository_1.CashBoxRepository.findById(boxId);
        if (!box)
            throw { status: 404, message: "Caja no encontrada" };
        if (box.status !== "OPEN")
            throw { status: 400, message: "Caja ya está cerrada" };
        return await prisma.$transaction(async (tx) => {
            ...
            const updateData = {
                status: client_1.CashBoxStatus.CLOSED,
                closedAt: new Date(),
            ...
            if (closeDto.cashCount !== undefined) {
                updateData.cashCount = closeDto.cashCount === null ? client_1.Prisma.JsonNull : closeDto.cashCount;
            }
    ...
    async getClosePreview(boxId) {
        ...
        if (!box)
            throw { status: 404, message: "Caja no encontrada" };
        if (box.status !== "OPEN" && box.status !== "REOPENED")
            throw { status: 400, message: "La caja ya está cerrada" };
    ...
    // Reabrir cajas
    async reopen(boxId, actorUserId) {
        // Verificar que la caja exista y esté cerrada
        const box = await cashBox_repository_1.CashBoxRepository.findById(boxId);
        if (!box)
            throw { status: 404, message: "Caja no encontrada" };
        if (box.status !== "CLOSED")
            throw { status: 400, message: "La caja ya está abierta" };
        // VERIFICACIÓN MODIFICADA: Solo verificar si hay caja abierta en la MISMA sucursal
        const openCashboxInBranch = await cashBox_repository_1.CashBoxRepository.findOpenByBranch(box.branchId);
    ...
    async closeReopened(boxId, actorUserId) {
        const box = await cashBox_repository_1.CashBoxRepository.findById(boxId);
        if (!box)
            throw { status: 404, message: "Caja no encontrada" };
        if (box.status !== "REOPENED")
            throw { status: 400, message: "Solo se pueden cerrar cajas reabiertas" };
        return await prisma.$transaction(async (tx) => {
