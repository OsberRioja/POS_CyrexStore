...
exports.ExchangeRateService = {
    ...
    async getRate(from, to) {
        ...
        to = to.toUpperCase();
        // Buscar en BD
        const rate = await prismaClient_1.prisma.exchangeRate.findUnique({
            where: {
    ...
    async updateFromAPI() {
        try {
            ...
            for (const currency of currencies) {
                if (rates[currency]) {
                    await prismaClient_1.prisma.exchangeRate.upsert({
                        where: {
                            fromCurrency_toCurrency: {
                                fromCurrency: 'USD',
                                toCurrency: currency
     ...
     */
    async updateManualRate(from, to, rate, userId, notes) {
        from = from.toUpperCase();
        to = to.toUpperCase();
        if (rate <= 0) {
            throw { status: 400, message: 'La tasa debe ser mayor a 0' };
        }
        return prismaClient_1.prisma.exchangeRate.upsert({
            where: {
     ...
     */
    async toggleManualRate(from, to, useManual) {
        from = from.toUpperCase();
        to = to.toUpperCase();
        const rate = await prismaClient_1.prisma.exchangeRate.findUnique({
            where: {
        ...
        return prismaClient_1.prisma.exchangeRate.update({
            where: {
                fromCurrency_toCurrency: { fromCurrency: from, toCurrency: to }
            },
            data: {
     ...
     */
    async listAll() {
        return prismaClient_1.prisma.exchangeRate.findMany({
            orderBy: [
                { fromCurrency: 'asc' },
                { toCurrency: 'asc' }
            ]
