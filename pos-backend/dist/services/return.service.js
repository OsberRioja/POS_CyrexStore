...
const return_repository_1 = require("../repositories/return.repository");
const returnPolicy_1 = require("../config/returnPolicy");
const prisma = new client_1.PrismaClient();
exports.ReturnService = {
    async createReturn(dto, actorUserId) {
        ...
        if (daysDiff > returnPolicy_1.RETURN_POLICY.maxDaysForReturn) {
            throw { status: 400, message: `La venta tiene ${daysDiff} días, no se puede devolver (máximo ${returnPolicy_1.RETURN_POLICY.maxDaysForReturn} días)` };
        }
        ...
        let status = client_1.ReturnStatus.PENDING;
        if (!returnPolicy_1.RETURN_POLICY.requiresApproval) {
        ...
        const returnRecord = await return_repository_1.ReturnRepository.create(returnData);
        // Si no requiere aprobación o si ya está aprobado, procesar inmediatamente
        if (status === client_1.ReturnStatus.APPROVED) {
            await this.processReturn(returnRecord.id, actorUserId);
        }
        return returnRecord;
    },
    async processReturn(returnId, actorUserId) {
        return await prisma.$transaction(async (tx) => {
            ...
            if (returnRecord.status !== client_1.ReturnStatus.APPROVED) {
            ...
            const updatedReturn = await tx.return.update({
                where: { id: returnId },
                data: { status: client_1.ReturnStatus.COMPLETED },
                include: {
                    items: {
                        include: {
                            product: {
    ...
    async approveReturn(returnId, actorUserId) {
        const returnRecord = await return_repository_1.ReturnRepository.findById(returnId);
        if (!returnRecord) {
            throw { status: 404, message: "Devolución no encontrada" };
        }
    ...
    async getReturnById(id) {
        const returnRecord = await return_repository_1.ReturnRepository.findById(id);
        if (!returnRecord) {
            throw { status: 404, message: "Devolución no encontrada" };
        }
        return returnRecord;
    ...
    async listReturns(params) {
        return return_repository_1.ReturnRepository.list(params);
    },
};
