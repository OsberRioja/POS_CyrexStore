...
exports.dashboardService = {
    ...
    async getBranchDashboard(branchId, period = 'day', date) {
        const targetDate = date || new Date();
        const { startDate, endDate } = this.getDateRange(period);
        // Obtener información de la sucursal
        const branch = await prismaClient_1.prisma.branch.findUnique({
        ...
        const salesInPeriod = await prismaClient_1.prisma.sale.findMany({
            where: {
                branchId,
                createdAt: {
                    gte: startDate,
        ...
        const cashBox = await prismaClient_1.prisma.cashBox.findFirst({
            where: {
                branchId,
                status: "OPEN",
                openedAt: {
        ...
        const lowStockProducts = await prismaClient_1.prisma.product.findMany({
            where: {
                branchId,
                stock: {
                    lt: 10
        ...
        const recentSales = await prismaClient_1.prisma.sale.findMany({
            where: {
                branchId
            },
            include: {
        ...
        const totalClients = await prismaClient_1.prisma.cliente.count();
        const totalProducts = await prismaClient_1.prisma.product.count({
    ...
    async getGeneralDashboard(period = 'day', date) {
        const targetDate = date || new Date();
        const { startDate, endDate } = this.getDateRange(period);
        // 1. Obtener todas las sucursales activas
        const branches = await prismaClient_1.prisma.branch.findMany({
            where: { isActive: true },
            select: { id: true, name: true }
        });
        // 2. Resumen global
        const activeBranches = branches.length;
        const totalBranches = await prismaClient_1.prisma.branch.count();
        ...
        const activeUsersInPeriod = await prismaClient_1.prisma.user.findMany({
            where: {
                Sale: {
                    some: {
                        createdAt: {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const daySales = await prismaClient_1.prisma.sale.findMany({
                where: {
                    createdAt: {
                        gte: dayStart,
                        lte: dayEnd
        ...
        for (const branch of branches) {
            ...
            const openCashBox = await prismaClient_1.prisma.cashBox.findFirst({
                where: {
                    branchId: branch.id,
                    status: "OPEN"
                }
            ...
            const hasSalesToday = await prismaClient_1.prisma.sale.findFirst({
                where: {
                    branchId: branch.id,
                    createdAt: {
                        gte: todayStart,
            ...
            const lowStockCount = await prismaClient_1.prisma.product.count({
                where: {
                    branchId: branch.id,
                    stock: { lt: 5 },
                    isActive: true
