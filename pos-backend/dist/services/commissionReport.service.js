...
exports.CommissionReportService = void 0;
const commissionReport_repository_1 = require("../repositories/commissionReport.repository");
...
exports.CommissionReportService = {
     ...
     */
    async getCommissionsByUserAndMonth(userId, month, year) {
        // Validar mes y año
        if (month < 1 || month > 12) {
        ...
        return await commissionReport_repository_1.CommissionReportRepository.findByUserAndMonth(userId, month, year);
    },
    /**
    ...
    async getSummaryByMonth(month, year) {
        ...
        const summary = await commissionReport_repository_1.CommissionReportRepository.getSummaryByMonth(month, year);
        // Obtener detalles de usuarios para el resumen
        const userDetails = await Promise.all(summary.map(async (item) => {
            const user = await prisma.user.findUnique({
    ...
    async getCommissionsByMonth(month, year, page = 1, limit = 50) {
        ...
        return await commissionReport_repository_1.CommissionReportRepository.getCommissionsByMonth(month, year, page, limit);
    },
    /**
     * Obtener reporte de comisiones por usuario en un rango de fechas
     */
    async getUserCommissionsReport(userId, startDate, endDate) {
        ...
        const summary = await commissionReport_repository_1.CommissionReportRepository.getUserCommissionsSummary(userId, start, end);
