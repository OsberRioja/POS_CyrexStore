...
exports.reportService = {
    async generateSalesReport(cashBoxId) {
        // Verificar que la caja existe y está cerrada
        const cashBox = await prismaClient_1.prisma.cashBox.findUnique({
            where: { id: cashBoxId },
            include: {
                sales: {
                    include: {
        ...
        cashBox.sales.forEach((sale) => {
            const products = sale.items.map(item => `${item.product.name} (x${item.quantity})`).join(', ');
            const totalQuantity = sale.items.reduce((sum, item) => sum + item.quantity, 0);
            const paymentMethods = sale.payments.map(payment => `${payment.paymentMethod.name}: Bs. ${payment.amount.toFixed(2)}`).join('\n');
    ...
    async generateExpensesReport(cashBoxId) {
        // Verificar que la caja existe y está cerrada
        const cashBox = await prismaClient_1.prisma.cashBox.findUnique({
            where: { id: cashBoxId },
            include: {
    ...
    async generatePaymentMethodsReport(cashBoxId) {
        // Verificar que la caja existe y está cerrada
        const cashBox = await prismaClient_1.prisma.cashBox.findUnique({
            where: { id: cashBoxId }
        });
        ...
        const paymentMethodsSummary = await prismaClient_1.prisma.paymentMethod.findMany({
            include: {
                salePayments: {
                    where: {
                        sale: {
        ...
        const totalGeneral = paymentMethodsSummary.reduce((sum, method) => {
            const methodTotal = method.salePayments.reduce((methodSum, payment) => methodSum + payment.amount, 0);
        ...
        paymentMethodsSummary.forEach((method) => {
            const methodTotal = method.salePayments.reduce((sum, payment) => sum + payment.amount, 0);
        ...
        const totalTransactions = paymentMethodsSummary.reduce((sum, method) => sum + method.salePayments.length, 0);
        const cashTotal = paymentMethodsSummary
            .filter(method => method.isCash)
            .reduce((sum, method) => sum + method.salePayments.reduce((methodSum, payment) => methodSum + payment.amount, 0), 0);
        ...
        const infoData = [
            ...
            { field: 'Cantidad de Métodos', value: paymentMethodsSummary.filter(m => m.salePayments.reduce((sum, p) => sum + p.amount, 0) > 0).length },
    ...
    async generateDailyReport(cashBoxId) {
        // Verificar que la caja existe y está cerrada
        const cashBox = await prismaClient_1.prisma.cashBox.findUnique({
            where: { id: cashBoxId },
            include: {
        ...
        const statsData = [
            ...
            ['Unidades Vendidas:', cashBox.sales.reduce((sum, sale) => sum + sale.items.reduce((itemSum, item) => itemSum + item.quantity, 0), 0)]
        ];
    ...
    async generateMonthlySalesReport(filters) {
        ...
        const totalPaid = sales.reduce((sum, sale) => sum + sale.payments.reduce((paidSum, payment) => paidSum + payment.amount, 0), 0);
    ...
    async generatePeriodSalesReport(filters) {
    ...
    async getAvailableSellers(branchId) {
    ...
    async getPeriodSalesPreview(filters) {
        const { startDate, endDate, branchId, sellerId, sellerIds, paymentMethodId } = filters;
        const adjustedStartDate = new Date(startDate);
        adjustedStartDate.setHours(0, 0, 0, 0);
        const adjustedEndDate = new Date(endDate);
        ...
        const totalSales = sales.length;
        const totalAmount = sales.reduce((sum, sale) => sum + sale.total, 0);
        const totalPaid = sales.reduce((sum, sale) => sum + sale.payments.reduce((pSum, payment) => pSum + payment.amount, 0), 0);
        const totalBalance = totalAmount - totalPaid;
        const averageTicket = totalSales > 0 ? totalAmount / totalSales : 0;
        const totalUnits = sales.reduce((sum, sale) => sum + sale.items.reduce((iSum, item) => iSum + item.quantity, 0), 0);
    ...
    async getPeriodExpensesPreview(filters) {
    ...
    async getCombinedPreview(filters) {
        ...
        salesPreview.rankings.branches.forEach((branch) => {
        ...
        expensesPreview.rankings.branches.forEach((branch) => {
